...
Export the signed `.app` from the archive (via Xcode's Organizer, or `xcodebuild -exportArchive` with an `ExportOptions.plist`), then package it as a `.zip` or `.dmg` for distribution.

## 3. Notarize

```sh
xcrun notarytool submit Studiofront.zip \
...
Re-zip/re-package after stapling.

## 4. Generate the appcast

Sparkle's EdDSA public key is already embedded in `app/project.yml` (`SUPublicEDKey`), so the matching private key must already exist in your Keychain from prior setup. Use Sparkle's `generate_appcast` tool (built as part of the Sparkle package, or downloaded from a [Sparkle release](https://github.com/sparkle-project/Sparkle/releases)) against a directory containing the packaged release artifact:

```sh
generate_appcast /path/to/release-artifacts/
```

This produces `appcast.xml` signed with the existing key.
...
- Download the released artifact fresh and confirm it opens without a Gatekeeper warning
